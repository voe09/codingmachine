...
class ListNode:
    ...
    def __init__(self, key: int, val: int, weight: int):
        self.key = key
        self.val = val
        self.weight = weight
        self.prev = None
...
class LRU:
    ...
    def __init__(self, capacity: int):
        self.cap = capacity
        self.count = 0
        self.front = ListNode(0, 0, 0)
        self.back = ListNode(0, 0, 0)
        self.front.next, self.back.prev = self.back, self.front
        self.data: dict[int, ListNode] = {}
    ...
    def get(self, key: int) -> Optional[int]:
        ...
        node = self.data[key]
        self._delete(node)
        self._insert(self.front, node)
        return node.val

    def set(self, key: int, val: int, weight: int) -> None:
        if weight > self.cap:
            return None
    
        if key in self.data:
            self._delete(self.data[key])

        while self.count + weight > self.cap:
            self._delete(self.back.prev)
        
        node = ListNode(key, val, weight)
        self._insert(self.front, node)
    ...
    def _delete(self, node: ListNode) -> None:
        ...
        self.count -= node.weight
    
    def _insert(self, pos: ListNode, node: ListNode) -> None:
        nxt = pos.next
        pos.next, node.next = node, nxt
        nxt.prev, node.prev = node, pos
...
if __name__ == "__main__":
    ...
    lru.set(2, 2, 1)
    print(lru.get(1)) # 1
    lru.set(3, 3, 4) # pop out 2
    print(lru.get(2)) # None
    lru.set(4, 4, 6) # Do nothing
    print(lru.get(4)) # None
    print(lru.get(1)) # 1
    print(lru.get(3)) # 3
    lru.set(1, 5, 4) # pop out 3 
    print(lru.get(3)) # None
    print(lru.get(1)) # 5
