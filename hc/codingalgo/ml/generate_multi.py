...
NUM_PROMPTS = 10000
NUM_LABELERS = 100
FILENAME = "hard_binary_data.json"

# 1. Define Labels (Binary & Unbalanced)
LABELS_BINARY = ["not_toxic", "toxic"]
LABEL_WEIGHTS = [0.75, 0.25]  # 75% not_toxic, 25% toxic
...
# 3. Simple prompt templates (same as before)
PROMPT_TEMPLATES = {
    "not_toxic": [
...
def simulate_labeler_choice(true_label):
    ...
    Now only 80% chance they pick the 'true' label.
    20% chance they flip the label (increased from 5%).
    This will create many more 2-1 splits.
    """
...
print(f"Generating {NUM_PROMPTS} 'hard' binary-class prompts...")
all_prompts = []
...
for i in range(NUM_PROMPTS):
    ...
    for labeler_id in labeler_ids:
        ...
        annotations.append({
            ...
            "label": simulate_labeler_choice(true_label), # Uses the new 20% error rate
            "labeler_metadata": {
...
print(f"Labeler error rate increased to 20% to create more 2-1 splits.")