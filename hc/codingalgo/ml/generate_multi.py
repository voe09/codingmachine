...
NUM_PROMPTS = 10000
NUM_LABELERS = 100
FILENAME = "large_multi_class_data.json"

# 1. Define Labels (Unbalanced)
# We set weights to make 'not_toxic' far more common.
LABELS_MULTICLASS = ["not_toxic", "mildly_toxic", "moderately_toxic", "highly_toxic", "severely_toxic"]
LABEL_WEIGHTS = [0.85, 0.08, 0.04, 0.02, 0.01]  # 85% are 'not_toxic'
...
# 3. Simple prompt templates to match the "true" label
PROMPT_TEMPLATES = {
    "not_toxic": [
    ...
    "mildly_toxic": [
        ...
        "This is the {adjective} worst service ever."
    ],
    "moderately_toxic": [
    ...
    "highly_toxic": [
        ...
        "People like you deserve to {action}."
    ],
    "severely_toxic": [
        "I'm going to find where you {action} and {violent_action} you.",
        "I will {violent_action} your entire {group}.", "Kill all {group}."
    ]
}
...
def get_random_prompt(label):
    ...
    # Just a simple placeholder replacement for variety
    return (
...
def simulate_labeler_choice(true_label):
    """
    Simulates labeler error.
    90% chance they pick the 'true' label.
    10% chance they pick a different, random label (simulating human error).
    """
    if random.random() < 0.90:
        return true_label
    else:
        # Pick a different label to simulate error
        error_label = random.choice(LABELS_MULTICLASS)
        while error_label == true_label:
            error_label = random.choice(LABELS_MULTICLASS)
        return error_label

print(f"Generating {NUM_PROMPTS} multi-class prompts...")
all_prompts = []
